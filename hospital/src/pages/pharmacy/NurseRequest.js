...
function NurseRequest({handleBack, setcurrentIndex, currentIndex}) {
    ...
    useEffect(()=>{
        const func =async()=>{
            try {
                await axios.post(`http://${'localhost'}:7700/utilityRequest`, {uid}).then((res)=>{
                    //console.log(res);
  ...
  return (
    <div className='dashboard_body' >
        ...
        <div className='dashboard_body_patient_details_btns'>
            <button className={currentIndex === 1 && 'dashboard_body_patient_details_btns_'} >UTILITY REQUESTS</button>
        ...
        
        {
            checkOut?.length > 0 ?
                ...
                const formatted3 = new Intl.NumberFormat('en-NG', {
                    ...
                    return(
                    <div key={i}>
                        <h4>Nurse {getstaff?.name} Request</h4>
                        { 
                        item?.type !== 'lab' &&
                            item?.type !== 'scan' &&
