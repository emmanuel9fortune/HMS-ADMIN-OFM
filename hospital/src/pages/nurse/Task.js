...
function Task() {
  ...
  const addTask =async()=>{
    ...
    try {
      await axios.post(`http://${'localhost'}:7700/addtask`, {tasks, uid: getid?._id}).then((res)=>{
    ...
    const handleUpdateTask =async(ids)=>{
        try {
        await axios.post(`http://${ip?.ip || 'localhost'}:7700/updatetask`, {taskId: ids, uid: getid?._id }).then((res)=>{
            if(res.data.status === 'success'){
              dispatch(
                setreloads(Date.now())
              )
