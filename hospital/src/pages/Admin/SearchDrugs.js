...
function SearchDrugs() {
    ...
    const handleExpDate =async(e)=>{
        ...
        setexpireDate(selectDate)

        try {
            await axios.post(`http://${cip || 'localhost'}:7700/editdrug`, {name: drugname, oprice, sprice, originalQuantity, quantity, expireDate: unixTimeSec, batch, limit, explimit, id: getItems?._id}).then((res)=>{
                setreload(reload + 1)
            })
        } catch (error) {
            console.log(error);
    ...
    const handleExpLimit =async(e)=>{
        ...
        setexplimit(selectDate)

        try {
            await axios.post(`http://${cip || 'localhost'}:7700/editdrug`, {name: drugname, oprice, sprice, originalQuantity, quantity, expireDate, batch, limit, explimit: unixTimeSec, id: getItems?._id}).then((res)=>{
                setreload(reload + 1)
            })
        } catch (error) {
            console.log(error);
