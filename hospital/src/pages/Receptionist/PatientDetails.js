...
function PatientDetails({setview}) {
    ...
    useEffect(()=>{
        const func=async()=>{
            try {
                await axios.post(`http://${'localhost'}:7700/getPatientDetails`, {uid: id?.uid}).then((res)=>{
                    //console.log(res);
    ...
    const handleSave =async()=>{
        try {
            await axios.post(`http://${'localhost'}:7700/editPatientDetails`, {changes: changes, uid: id?.uid}).then((res)=>{
                if(res.data.status === 'success'){
                    setreload(reload + 1)
