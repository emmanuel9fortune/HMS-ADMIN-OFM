...
router.post('/', async(req, res) => {
    try {
        ...
        const getNotification = await notifications.find({role: 'doctor'}).sort({timeStamp: -1}).limit(10)

        const getPatientsID = await getNotification?.length > 0 ? getNotification?.map((item)=> item?.uid) : []

        const getPatientDetails = await Patient.find({_id: {$in: getPatientsID}}).select('name phone status center hop familyid family _id')
