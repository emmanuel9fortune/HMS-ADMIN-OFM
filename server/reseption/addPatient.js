...
const { Patient, antenatal, patientEdited, queue, notifications, billRequests, AnteNat, cards } = require('../../model');
                ...
                if(hours >= 18){
                    await billRequests.create({
                        ...
                        services: JSON.stringify({
                            items:[{
                                name: 'Evening Card', 
                                price: 
                                card?.eveningCardAmount ? 
                                card?.eveningCardAmount : 1500, 
                    ...
                    await billRequests.create({
                        ...
                        services: JSON.stringify({
                            items:[{
                                name: 'Morning Card', 
                                price: card?.morningCardAmount ? card?.morningCardAmount : 500, 
                                quantity: 1, 
                                totalPrice: card?.morningCardAmount ? card?.morningCardAmount : 500
                            }], 
                            totalPrice: card?.morningCardAmount ? card?.morningCardAmount : 500
                        }),
                        status: 'PENDING',
            ...
            if(family && patientType1 === 'new'){
                await billRequests.create({
                    ...
                    services: JSON.stringify({
                        items:[{
                            name: 'Family Card', 
                            price: card?.familyCardAmount ? card?.familyCardAmount : 5000, 
                            quantity: 1, 
                            totalPrice: card?.familyCardAmount ? card?.familyCardAmount : 5000
                        }] , 
                        totalPrice: card?.familyCardAmount ? card?.familyCardAmount : 5000
                    }),
                    status: 'PENDING',
